...
{
    uint64_t result = 1; // mpz_set_ui( result, 1 );
    if( n < 2 )
        return result;
    while( n > 1 )
    {
...
int largest_combination( size_t size, unsigned data[] )
{
    unsigned *aComb = new unsigned[ size ]; // temp set
    uint64_t  nComb = factorial( size );
    unsigned  nBest = 0;
...
printf( "Combinations: %u\n", (unsigned) nComb );
    ...
    for( uint64_t iComb = 0; iComb < nComb; iComb++ )
    {
        // Reset temp set
...
printf( "Combination: #%u\n", (unsigned) iComb );
        ...
        for( size_t iDigit = 0; iDigit < size; iDigit++    )
        {
            ...
            unsigned n     = aComb[ digit ];
            unsigned nTens = num_digits( n );
...
int main()
{
    //unsigned DATA[] = { 50,  2,   1, 9 }; // 9+50+2+1
    //unsigned DATA[] = {  5, 50,  56    }; // 56+5+50
    //unsigned DATA[] = {420, 42, 423    }; // 42+423+420
      unsigned DATA[] = { 52,  5,   3    }; // 5+52+3
    //unsigned DATA[] = {  1,  2,   3    }; // 3+2+1
